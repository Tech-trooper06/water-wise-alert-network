...
export const generateSensorLocations = (count = 15) => {
  ...
  return Array.from({ length: count }).map((_, i) => {
    ...
    return {
      id: `sensor-${i + 1}`,
      lat: faker.number.float({ min: 10, max: 90, precision: 0.01 }),
      lng: faker.number.float({ min: 10, max: 90, precision: 0.01 }),
...
export const generateWaterQualityData = () => {
  return {
    ...
    metrics: [
      {
        label: 'pH Level',
        value: faker.number.float({ min: 6.0, max: 8.5, precision: 0.1 }),
        max: 14,
        unit: 'pH',
      ...
      {
        label: 'Turbidity',
        value: faker.number.float({ min: 0.5, max: 10, precision: 0.1 }),
        max: 20,
        unit: 'NTU',
        status: faker.helpers.arrayElement(['good', 'warning', 'critical']) as 'good' | 'warning' | 'critical',
      },
      {
        ...
        value: faker.number.float({ min: 2, max: 12, precision: 0.1 }),
        max: 15,
        unit: 'mg/L',
        status: faker.helpers.arrayElement(['good', 'warning', 'critical']) as 'good' | 'warning' | 'critical',
      },
